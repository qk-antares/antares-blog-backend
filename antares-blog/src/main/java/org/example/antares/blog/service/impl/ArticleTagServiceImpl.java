...
public class ArticleTagServiceImpl extends ServiceImpl<ArticleTagMapper, ArticleTag>
    ...
    public R getAllTags() {
        ...
        //获取标签
        List<ArticleTagCategoryVo> vos = categories.stream().map(category -> {
    ...
    @Override
    public R addATag(ArticleTagAddRequest articleTagAddRequest, HttpServletRequest request) {
        ...
        for (ArticleTag tag : tags) {
            if(tag.getName().equals(articleTagAddRequest.getName())){
                return R.error(AppHttpCodeEnum.USER_TAG_EXIST);
            }
        }
